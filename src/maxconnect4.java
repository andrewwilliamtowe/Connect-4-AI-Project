...
public class maxconnect4
{
  public static void main(String[] args) 
  {
    ...
    String whoseturn=args[2].toString();
    ...
    if( game_mode.equalsIgnoreCase( "interactive" ) ) 
    {
        ...
        if(whoseturn.equals("computer-next")){
            computerfirst=true;
        }else{
            computerfirst=false;
        }
	while(!(currentGame.getPieceCount()==42)){
        if(computerfirst){
            currentGame.playPiece(calculon.AIMove(currentGame,1));
            currentGame.playPiece(calculon.PCMove(currentGame));
        }else{
            currentGame.playPiece(calculon.PCMove(currentGame));
            currentGame.playPiece(calculon.AIMove(currentGame,2));
        }
    }
        currentGame.printGameBoard();
	...
	playColumn = calculon.findBestPlay( currentGame ,current_player);
